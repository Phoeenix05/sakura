//! kudos to [hoppscotch](`https://github.com/hoppscotch/hoppscotch`)
...
pub fn setup_mac_window(app: &mut App) {
    ...
    fn with_hopp_app<F: FnOnce(&mut SakuraAppState) -> T, T>(this: &Object, func: F) {
        let ptr = unsafe {
            let x: *mut c_void = *this.get_ivar("sakuraApp");
            &mut *(x as *mut SakuraAppState)
        };
        func(ptr);
    }
    ...
    unsafe {
        ...
        let app_state = SakuraAppState { window };
        let app_box = Box::into_raw(Box::new(app_state)) as *mut c_void;

        ns_win.setDelegate_(delegate!("MainWindowDelegate", {
            window: id = ns_win,
            sakuraApp: *mut c_void = app_box,
    ...
    app.listen("sakura-bg-changed", move |ev| {
